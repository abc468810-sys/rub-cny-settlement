...
const MOCK_DATA = [
  { month: 'OCT', volume: 1200000 },
  { month: 'NOV', volume: 1500000 },
  { month: 'DEC', volume: 1850000 },
  { month: 'JAN', volume: 2100000 },
  { month: 'FEB', volume: 1950000 },
  { month: 'MAR', volume: 2450000 },
];

export default function MonthlyVolumeAnalytics() {
  const max = Math.max(...MOCK_DATA.map(d => d.volume));
        ...
        {MOCK_DATA.map((d, i) => (
          <div key={d.month} className="flex-1 flex flex-col items-center group/bar relative h-full justify-end">
            ...
            <motion.div 
              ...
              initial={{ height: 0 }}
              animate={{ height: `${(d.volume / max) * 100}%` }}
            ...
            <p className="text-[9px] font-black mt-6 text-gray-300 uppercase tracking-[0.2em] group-hover/bar:text-black transition-colors">{d.month}</p>
          </div>
        ))}
      </div>
      
      <footer className="pt-8 border-t border-gray-50 flex justify-between items-center text-[9px] font-black uppercase tracking-widest text-gray-300 px-2">
